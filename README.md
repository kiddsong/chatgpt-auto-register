...
当然，它在注册过程中你不用管它，你可以在副屏上或者屏幕的其他位置做别的事情。按道理来说其实是可以最小化运行的，但是作者在测试的时候似乎发生了错误，所以干脆就让它在屏幕的右上角自动运行了。不需要保持窗口的活跃状态。
...
**特别注意**强烈建议你的CLI PROXY API面板IP和你注册账号时的IP保持一致，否则注册到的账号一定会被封。
